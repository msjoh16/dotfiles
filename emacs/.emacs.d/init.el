...
(defvar strider/notebook-dir
  (cond
   ((eq system-type 'darwin)
    "/Users/adamsjoholm/Library/Mobile Documents/com~apple~CloudDocs/files/active/notebook")
   ((eq system-type 'gnu/linux)
    "~/files/")
   (t "~/files/"))  ; fallback
  "Base directory for notebook files.")

(setq default-directory strider/notebook-dir)
...
(use-package elfeed
  ...
  ;; Custom faces
  (defface elfeed-search-keep-face
    '((t :foreground "#4d772d"))
    "Face for kept elfeed entries.")
  (defface elfeed-search-drop-face
    '((t :foreground "#e01d1d"))
    "Face for dropped elfeed entries.")

  ;; Hook to apply faces based on tags
  (add-hook 'elfeed-search-update-hook
            (lambda ()
              (face-remap-add-relative 'elfeed-search-unread-title-face
                                       'default)))

  ;; Tag functions
  (defun strider/elfeed-tag-keep ()
    (interactive)
    (elfeed-search-toggle-all 'keep))
  (defun strider/elfeed-tag-drop ()
    (interactive)
    (elfeed-search-toggle-all 'drop))

  ;; Apply faces via search-face-alist
  (push '(keep elfeed-search-keep-face) elfeed-search-face-alist)
  (push '(drop elfeed-search-drop-face) elfeed-search-face-alist))
...
(use-package elfeed-org
  ...
  (setq rmh-elfeed-org-files
        (list (concat strider/notebook-dir "/notes/rss_feeds.org")))
  (elfeed-org))
...
(use-package general
  ...
  (strider/leader-keys
    ...
    ;; Elfeed
    "o" '(:ignore r :which-key "open")
    "of" '(elfeed :which-key "rss feeds")
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(all-the-icons biblio calfw-org citar counsel doom-modeline
		   doom-themes elfeed-org evil general helpful
		   ivy-rich magit ob-mermaid org-bullets ox-pandoc
		   visual-fill-column)))
(custom-set-faces
